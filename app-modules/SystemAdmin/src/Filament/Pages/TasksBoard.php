...
final class TasksBoard extends BoardPage
{
    ...
    /**
     * Configure the board for SystemAdmin panel (no tenant scoping).
     */
        ...
        return $board
            ->query(
                Task::query()
                    ...
                    ->with(['assignees', 'team']) // Eager load relationships
            )
            ...
            ->columnActions([
                CreateAction::make()
                    ...
                    ->using(function (array $data, array $arguments): Task {
                        // For SystemAdmin, we need to ensure team_id is set
    ...
    private function getColumns(): array
    {
        return $this->statuses()->map(
            fn(array $status): Column => Column::make((string) $status['id'])
                ->color($status['color'])
                ->label($status['name'])
        )->toArray();
    }
